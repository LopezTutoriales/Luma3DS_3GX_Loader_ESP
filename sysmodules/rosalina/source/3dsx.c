...
bool Ldr_Get3dsxSize(u32* pSize, IFile *file)
{
    ...
    {
        Log_PrintP("No se puede leer header de 3DSX");
        return false;
    }
...
Handle Ldr_CodesetFrom3dsx(const char* name, u32* codePages, u32 baseAddr, IFile *file, u64 tid)
{
                    ...
                    for (m = 0; m < nPatches && pos < endPos; m ++)
                    {
                        ...
                        switch (j)
                        {
                            case 0:
                            {
                                if (subType != 0)
                                {
                                    Log_PrintP("Absolute reloc subtype no soportado (%lu)", subType);
                                    return 0;
                                }
                                *pos = addr;
                                break;
                            ...
                            case 1:
                            {
                                ...
                                switch (subType)
                                {
                                    ...
                                    default:
                                        Log_PrintP("Relative reloc subtype no soportado (%lu)", subType);
                                        return 0;
                                }
                                break;
                            }
    ...
    if (res)
    {
        Log_PrintP("svcCrearConjuntoCodigo: %08lX", res);
        return 0;
    }
